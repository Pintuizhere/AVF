...
export default function FeaturedSection() {
  ...
  const scroll = (direction) => {
    [scrollContainerRefVertical.current, scrollContainerRefHorizontal.current].forEach(container => {
      if (container) {
  ...
  return (
    <section className="bg-black pt-10 md:pt-20 text-white relative overflow-hidden">
      ...
      {/* SHORTS BLOCK (Vertical) */}
      {shortsItems.length > 0 && (
        <div className="container mx-auto px-6 max-w-5xl relative group z-10 mb-12">

          <div 
            ref={scrollContainerRefVertical}
            className="flex gap-4 md:gap-8 overflow-x-auto pb-6 pt-2 snap-x snap-mandatory scrollbar-hide -mx-6 px-6 md:mx-0 md:px-0 scroll-pl-6 md:scroll-pl-0"
            style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
          >
            {shortsItems.map((reel, index) => {
              const content = (
                <>
      ...
      {isVisible && featuredItems.length > 0 && (
        <div className="container mx-auto px-6 max-w-5xl relative group z-10 mb-8">
          ...
          <div 
            ref={scrollContainerRefHorizontal}
